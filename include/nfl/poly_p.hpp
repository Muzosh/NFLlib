...
namespace nfl
{
    ...
    public:
        poly_p(poly_p const &o) : _p(o._p)
        {
        }

        // AG: we need to declare this one or it will be catched by the generic one
        // below!
        poly_p(poly_p &o) : _p(const_cast<poly_p const &>(o)._p)
        {
        }

        poly_p(poly_p &&o) : _p(std::move(o._p))
        {
        }

        template <class... Args>
        poly_p(Args &&...args) : _p(make_pointer(std::forward<Args>(args)...))
        {
        }
    ...
    public:
        ...
        void set(gaussian<in_class, T, _lu_depth> const &mode) { poly_obj().set(mode); };
        ...
        template <class It>
        void set(It first, It last, bool reduce_coeffs = true) { poly_obj().set(first, last, reduce_coeffs); };
    ...
    public:
        ...
        template <class It>
        void set_mpz(It first, It last) { poly_obj().set_mpz(first, last); };
