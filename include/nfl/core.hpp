...
namespace nfl
{
    ...
    inline poly<T, Degree, NbModuli>::poly(ops::expr<Op, Args...> const &expr)
    {
        *this = expr;
    }
    ...
    template <class T, size_t Degree, size_t NbModuli>
    template <class Op, class... Args>
    inline poly<T, Degree, NbModuli> &poly<T, Degree, NbModuli>::operator=(ops::expr<Op, Args...> const &expr)
    {
    ...
    template <class T, size_t Degree, size_t NbModuli>
    poly<T, Degree, NbModuli>::core::core()
    {
    ...
    template <class T, size_t Degree, size_t NbModuli>
    bool poly<T, Degree, NbModuli>::core::ntt(value_type *x, const value_type *wtab, const value_type *winvtab, value_type const p)
    {
#ifdef CHECK_STRICTMOD
        for (size_t i = 0; i < degree; i++)
        {
    ...
    template <class T, size_t Degree, size_t NbModuli>
    inline bool poly<T, Degree, NbModuli>::core::inv_ntt(value_type *x, const value_type *const inv_wtab, const value_type *const inv_winvtab,
    ...
    template <class T, size_t Degree, size_t NbModuli>
    inline void poly<T, Degree, NbModuli>::core::prep_wtab(value_type *wtab, value_type *wtabshoup, value_type w, size_t cm)
    {
        auto const p = get_modulus(cm);
        unsigned K = degree;
    ...
    template <class T, size_t Degree, size_t NbModuli>
    inline void poly<T, Degree, NbModuli>::core::ntt_pow_phi(poly &op)
    {
    ...
    template <class T, size_t Degree, size_t NbModuli>
    inline void poly<T, Degree, NbModuli>::core::invntt_pow_invphi(poly &op)
    {
        for (size_t currentModulus = 0; currentModulus < nmoduli; ++currentModulus)
        {
    ...
    template <class T, size_t Degree, size_t NbModuli>
    void poly<T, Degree, NbModuli>::core::initialize()
    {
        for (size_t currentModulus = 0; currentModulus < nmoduli; ++currentModulus)
        {
            ...
            for (unsigned int i = 0; i < degree; i++)
            {
                ...
                shoupinvpoly_times_invphis[currentModulus][i] = ((greater_value_type)temp << params<T>::kModulusRepresentationBitsize) / get_modulus(currentModulus);
