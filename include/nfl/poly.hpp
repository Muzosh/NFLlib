...
namespace nfl
{
    ...
    class poly
    {
        ...
        template <class P>
        friend class tests::poly_tests_proxy;
    ...
    public:
        ...
        template <class It>
        poly(It first, It last, bool reduce_coeffs = true);
        template <class Op, class... Args>
        poly(ops::expr<Op, Args...> const &expr);
        template <class in_class, unsigned _lu_depth>
        poly(gaussian<in_class, T, _lu_depth> const &mode);
        ...
        template <class It>
        void set(It first, It last, bool reduce_coeffs = true);
        template <class in_class, unsigned _lu_depth>
        void set(gaussian<in_class, T, _lu_depth> const &mode);

        /* assignment
         */
        poly &operator=(value_type v)
        {
            set(v);
            return *this;
        }
        poly &operator=(uniform const &mode)
        {
            set(mode);
            return *this;
        }
        poly &operator=(non_uniform const &mode)
        {
            set(mode);
            return *this;
        }
        poly &operator=(hwt_dist const &mode)
        {
            set(mode);
            return *this;
        }
        poly &operator=(ZO_dist const &mode)
        {
            set(mode);
            return *this;
        }
        poly &operator=(std::initializer_list<value_type> values)
        {
            set(values);
            return *this;
        }
        template <class in_class, unsigned _lu_depth>
        poly &operator=(gaussian<in_class, T, _lu_depth> const &mode)
        {
            set(mode);
            return *this;
        }
        template <class Op, class... Args>
        poly &operator=(ops::expr<Op, Args...> const &expr);
        ...
        template <class M>
        auto load(size_t cm, size_t i) const -> decltype(M::load(&(this->operator()(cm, i)))) { return M::load(&(*this)(cm, i)); }
    ...
    public:
        ...
        template <class It>
        void set_mpz(It first, It last);

        poly &operator=(mpz_t const &v)
        {
            set_mpz(v);
            return *this;
        }
        poly &operator=(mpz_class const &v)
        {
            set_mpz(v);
            return *this;
        }
        poly &operator=(std::array<mpz_t, Degree> const &values)
        {
            set_mpz(values);
            return *this;
        }
        poly &operator=(std::array<mpz_class, Degree> const &values)
        {
            set_mpz(values);
            return *this;
        }
        poly &operator=(std::initializer_list<mpz_t> const &values)
        {
            set_mpz(values);
            return *this;
        }
        poly &operator=(std::initializer_list<mpz_class> const &values)
        {
            set_mpz(values);
            return *this;
        }
