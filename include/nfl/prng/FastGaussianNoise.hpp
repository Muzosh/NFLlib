...
namespace nfl
{
    ...
    inline double newton_raphson(double k, double max_guess, int digits)
    {
        ...
        for (unsigned counter = 0; counter < max_counter; counter++)
        {
            ...
            guess -= delta;
            if (fabs(delta) / fabs(guess) < pow(10.0, -digits))
                break;
        }
        // In case there is a flat zone in the function
        while (0.95 * guess * 0.95 * guess - 2 * log(0.95 * guess) - 1 - 2 * k * log(2) >= 0)
            guess *= 0.95;
        // Test result
    ...
    FastGaussianNoise<in_class, out_class, _lu_depth>::FastGaussianNoise(double sigma,
                                                                         unsigned int security, unsigned int samples, double center_d /*=0*/, bool verbose /*=true*/) : _sigma(sigma),
                                                                                                                                                                        _security(security),
                                                                                                                                                                        _samples(samples),
                                                                                                                                                                        _verbose(verbose)
    {
    ...
    template <class in_class, class out_class, unsigned _lu_depth>
    FastGaussianNoise<in_class, out_class, _lu_depth>::FastGaussianNoise(double sigma,
                                                                         unsigned int security, unsigned int samples, mpfr_t center /*=0*/, bool verbose /*=true*/) : _sigma(sigma),
                                                                                                                                                                      _security(security),
                                                                                                                                                                      _samples(samples),
                                                                                                                                                                      _verbose(verbose)
    {
    ...
    void FastGaussianNoise<in_class, out_class, _lu_depth>::init()
    {
        ...
        // Give some feedback
        if (_verbose)
            std::cout << "FastGaussianNoise: " << _number_of_barriers << " barriers with " << _bit_precision << " bits of precision will be computed" << std::endl;
    }
    ...
    void FastGaussianNoise<in_class, out_class, _lu_depth>::precomputeBarrierValues()
    {
        ...
        for (int i = 0; i < (int)_number_of_barriers; i++)
        {
            ...
            // std::cout << std::endl;
            if (i == 0)
                mpfr_set(mp_barriers[0], tmp, MPFR_RNDN);
            else
            {
        ...
        for (unsigned i = 0; i < _number_of_barriers; i++)
        {
            ...
            mpz_export((void *)(barriers[i] + ((int)_word_precision -
                                               (int)ceil((float)mpz_sizeinbase(int_value, 256) / sizeof(in_class)))),
                       nullptr, 1, sizeof(in_class), 0, 0, int_value);
#ifdef OUTPUT_BARRIERS
            mpz_out_str(stdout, 10, int_value);
            std::cout << " = Barriers[" << i << "] = " << std::endl;
            if (sizeof(in_class) == 1)
                for (unsigned j = 0; j < _word_precision; j++)
                    printf("%.2x", barriers[i][j]);
            if (sizeof(in_class) == 2)
                for (unsigned j = 0; j < _word_precision; j++)
    ...
    void FastGaussianNoise<in_class, out_class, _lu_depth>::buildLookupTables()
    {
        ...
        // Give some feedback
        if (_verbose)
            std::cout << "FastGaussianNoise: Lookup tables built" << std::endl;
    }
    ...
    void FastGaussianNoise<in_class, out_class, _lu_depth>::getNoise(out_class *const rand_outdata, uint64_t rlen)
    {
        ...
        used_words = 0;
        if (_verbose)
            std::cout << "FastGaussianNoise: Using "
                      << " " << innoise_multiplier * sizeof(in_class) * 8 / std::min(log2(_number_of_barriers), (double)sizeof(out_class) * 8) << " input bits per output bit" << std::endl;
        ...
        while (computed_outputs < rlen)
        {
            ...
            if ((used_words + _word_precision) >= innoise_words)
            {
                ...
                used_words = 0;
                if (_verbose)
                    std::cout << "FastGaussianNoise: All the input bits have been used, regenerating them ..." << std::endl;
    ...
    FastGaussianNoise<in_class, out_class, _lu_depth>::~FastGaussianNoise()
    {
        ...
        if (_lu_depth == 2)
        {
            ...
            {
                if (lu_table2[ctr] != nullptr)
                    delete[] (lu_table2[ctr]);
            }
            free(lu_table2);
        }
    }
